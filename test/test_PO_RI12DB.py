...
def obtener_fecha_proceso(dias_atras: int = 1) -> str:
    ...
    # fecha = "20260122" # Variable estática comentada
...
def analizar_por_sku(ktrhp_validas: dict, kpuhp: dict) -> list:
    ...
    resultado = []
    for sku, total_ktrhp in ktrhp_validas.items():
...
def main_vendor():
    logger.info("=== INICIO CONCILIACIÓN AS400 (MODO PRUEBA RI12DB) ===")
    BASE, fecha = "RI12DB", "20260309"
    owner_dinamico = BASE[2:4]
    valid_owner = f"V{BASE[3]}"
    ...
    NotificadorCorreo().enviar(asunto, cuerpo, adjuntos=[os.path.abspath(ruta_reporte)], destinatarios_explicitos=["eliseo_lopezp@unicomer.com"])
    
    logger.info("=== FIN CONCILIACIÓN AS400 (MODO PRUEBA RI12DB) ===")
