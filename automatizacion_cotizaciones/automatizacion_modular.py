...
def configurar_logging():
    ...
    # Nombre de archivo único con timestamp diario (sobrescribe si existe)
    ...
    # Handler para archivo - continúa escribiendo en el mismo archivo si existe
    file_handler = logging.FileHandler(
        log_file, 
        mode='a',              # 'a' para agregar/continuar escribiendo
        encoding='utf-8'
    )
...
#URL_LOGIN = "http://urgtorbs6dlk:8080/engage-unicomer-web/"
EMPLOYEE_ID = "pos"
PASSWORD = "pos1234"
TIMEOUT = 300
...
def esperar_y_verificar_visibilidad(driver, selector_tipo, selector_valor, timeout=300, descripcion="elemento"):
    """
    Función mejorada para esperar que un elemento sea visible y clickeable
    El timeout se reinicia cada vez que hay progreso. Solo se detiene si NO hay progreso durante todo el timeout
    """
    import time
    ...
    try:
        LOGGER.info(f"Esperando que {descripcion} sea visible (timeout: {timeout}s, se reinicia con progreso)...")
        
        while True:
            current_time = time.time()
            time_since_last_progress = current_time - last_progress_time
            
            # Si no hay progreso durante el timeout completo, quitar/salir del elemento
            if time_since_last_progress >= timeout:
                LOGGER.warning(f"Sin progreso en {descripcion} por {timeout}s - QUITANDO elemento y continuando")
                LOGGER.info(f"Total transcurrido: {current_time - start_time:.1f}s")
                return None  # Retornar None indica que se debe quitar/saltar este elemento
            
            try:
                # Verificar si el elemento existe
                element = driver.find_element(selector_tipo, selector_valor)
                
                # PROGRESO DETECTADO: elemento existe
                if time_since_last_progress > 5:  # Solo log si ha pasado tiempo significativo
                    LOGGER.info(f"PROGRESO: {descripcion} existe - REINICIANDO timeout")
                last_progress_time = current_time  # REINICIAR timeout
                
                if element.is_displayed() and element.is_enabled():
                    # PROGRESO DETECTADO: elemento visible y habilitado
                    LOGGER.info(f"PROGRESO: {descripcion} visible - REINICIANDO timeout")
                    last_progress_time = current_time  # REINICIAR timeout
                    
                    try:
                        # Verificar que también sea clickeable
                        ...
                        LOGGER.info(f"OK - {descripcion} listo después de {total_elapsed:.1f}s")
                        return element
                        
                    except TimeoutException:
                        # Existe y es visible, pero no clickeable aún
                        LOGGER.debug(f"{descripcion} visible pero no clickeable aún...")
                        pass
                        
            except (NoSuchElementException, StaleElementReferenceException):
                # Elemento no existe aún - esto es normal, seguir esperando
                pass
            
            # Esperar un poco antes del siguiente intento
            time.sleep(0.5)
            
            # Log de progreso cada 15 segundos desde último progreso
            if int(time_since_last_progress) % 15 == 0 and time_since_last_progress > 0:
                remaining = timeout - time_since_last_progress
                total_elapsed = current_time - start_time
                LOGGER.info(f"Esperando {descripcion}... Sin progreso: {time_since_last_progress:.0f}s, Total: {total_elapsed:.0f}s, Queda: {remaining:.0f}s")
        
    except Exception as e:
        LOGGER.error(f"Error inesperado verificando visibilidad de {descripcion}: {e}")
        return None  # En caso de error, quitar elemento

def click_seguro_cuando_visible(driver, selector_tipo, selector_valor, timeout=TIMEOUT, descripcion="elemento"):
    """
    Hace click solo cuando el elemento está visible y disponible
    Si no es visible después del timeout (sin progreso), quita el elemento y continúa
    """
    try:
        element = esperar_y_verificar_visibilidad(driver, selector_tipo, selector_valor, timeout, descripcion)
        
        # Si retorna None, significa que se agotó timeout sin progreso - quitar elemento
        if element is None:
            LOGGER.warning(f"QUITANDO: {descripcion} - Sin progreso por {timeout}s, continuando sin él")
            return False  # Retornar False para indicar que se quitó el elemento
        
        # Scroll al elemento si es necesario
        driver.execute_script("arguments[0].scrollIntoView(true);", element)
        time.sleep(0.5)  # Pequeña pausa después del scroll
        
        # Verificar nuevamente que sigue siendo visible después del scroll
        if not element.is_displayed():
            LOGGER.warning(f"{descripcion} se volvió invisible después del scroll - quitando")
            return False
        
        # Click usando JavaScript para evitar interceptación
        driver.execute_script("arguments[0].click();", element)
        LOGGER.info(f"Click exitoso en {descripcion}")
        return True
        
    except Exception as e:
        LOGGER.warning(f"Error en click seguro de {descripcion}: {e} - quitando elemento")
        return False

def limpiar_screenshots_cotizacion():
    """Limpia screenshots de la cotización actual y los incluye en el Word"""
    global SCREENSHOTS_TOMADAS, COTIZACION_ACTUAL
    try:
        LOGGER.info(f"Iniciando limpieza de screenshots para cotización {COTIZACION_ACTUAL}")
        if SCREENSHOTS_TOMADAS:
            screenshots_eliminados = 0
            LOGGER.info(f"Encontrados {len(SCREENSHOTS_TOMADAS)} screenshots para limpiar")
            for archivo, descripcion in SCREENSHOTS_TOMADAS:
                if os.path.exists(archivo):
                    try:
                        os.remove(archivo)
                        screenshots_eliminados += 1
                        LOGGER.info(f"Screenshot eliminado: {archivo}")
                    except Exception as e:
                        LOGGER.warning(f"Error eliminando {archivo}: {e}")
                else:
                    LOGGER.warning(f"Screenshot no existe: {archivo}")
            
            SCREENSHOTS_TOMADAS.clear()
            if screenshots_eliminados > 0:
                LOGGER.info(f"OK - {screenshots_eliminados} screenshots de cotización {COTIZACION_ACTUAL} eliminados")
        else:
            LOGGER.info("No hay screenshots para eliminar")
...
def iniciar_orden_compra():
    """Busca y hace click en botón 'Comience una Orden de Compra' - quita elementos que no responden"""
    global DRIVER
    try:
        LOGGER.info("Buscando botón 'Comience una Orden de Compra'...")
        capturar_screenshot("02_dashboard.png", "Dashboard principal")
        
        # Selectores basados en v2 funcional - XPath específico primero
        selectores = [
            (By.XPATH, "/html/body/div[2]/div[4]/div/div/div[1]/a[1]/input")        
        ]
        
        for i, (tipo, selector) in enumerate(selectores, 1):
            LOGGER.info(f"Intentando selector {i}: {selector}")
            
            # Usar función con timeout que se reinicia con progreso
            result = click_seguro_cuando_visible(DRIVER, tipo, selector, timeout=TIMEOUT, 
                                               descripcion=f"botón orden selector {i}")
            
            if result:  # Si retorna True, el click fue exitoso
                time.sleep(1)
                capturar_screenshot("04_formulario_orden.png", "Formulario orden abierto")
                LOGGER.info("Click realizado en botón orden")
                return True
            else:
                # Si retorna False, el elemento se quitó por timeout sin progreso
                LOGGER.info(f"Selector {i} quitado por falta de progreso, probando siguiente...")
                continue
        
        # Si llegamos aquí, ningún selector funcionó
        LOGGER.error("Ningún selector de botón orden funcionó - todos fueron quitados por timeout")
        return False
        
    except Exception as e:
        LOGGER.error(f"Error crítico iniciando orden: {e}")
        return False
...
def agregar_item(codigo_producto, cantidad=1, numero_producto=1):
    """
    Módulo 3: Agrega un item individual al carrito usando validaciones de visibilidad mejoradas
    """
    global DRIVER
    try:
        ...
        # Buscar campo de búsqueda con validación mejorada
        try:
            campo_busqueda = esperar_y_verificar_visibilidad(
                DRIVER, By.ID, "saleItemSearchInput", 
                timeout=TIMEOUT, 
                descripcion=f"campo búsqueda item {numero_producto}"
            )
        except (TimeoutException, ElementNotInteractableException) as e:
            LOGGER.error(f"Campo de búsqueda no disponible para item {numero_producto}: {e}")
            return False
        
        # Limpiar y agregar producto
        campo_busqueda.clear()
        campo_busqueda.send_keys(str(codigo_producto))
        LOGGER.info(f"Código {codigo_producto} ingresado en campo búsqueda")
        
        # Submit búsqueda con validación de visibilidad
        try:
            campo_busqueda.send_keys(Keys.RETURN)
            LOGGER.info("Enter enviado para búsqueda")
        except Exception:
            try:
                # Buscar botón de búsqueda si Enter falla
                if click_seguro_cuando_visible(DRIVER, By.CSS_SELECTOR, "input[type='submit'][value='Search']", 
                                             timeout=5, descripcion="botón búsqueda"):
                    LOGGER.info("Click en botón Search realizado")
                else:
                    LOGGER.warning("No se pudo hacer click en botón Search")
            except Exception as e:
                LOGGER.warning(f"Error con botón búsqueda: {e}")
        ...
        LOGGER.info(f"Item {codigo_producto} procesado correctamente")
        return True
...
def agregar_despacho(entrega_cliente, tipo_entrega):
    """
    ...
    try:
        ...
        if not con_despacho:
            LOGGER.info("Entrega cliente omitida (configurado como No)")
            return True
...
def modulo_7_aplicar_credito():
    ...
    LOGGER.info("=== MÓDULO 7: Creditos ==========================================================")
    opcion_aplicar_credito = WebDriverWait(DRIVER, TIMEOUT).until(
        EC.element_to_be_clickable((By.XPATH, "/html/body/div[2]/div[4]/div/div/ul/li[3]/a"))
    )
    opcion_aplicar_credito.click()

    opcion_aplicar_credito_boton = WebDriverWait(DRIVER, TIMEOUT).until(
        EC.element_to_be_clickable((By.XPATH, "/html/body/div[2]/div[4]/div/div/ul/form/li/button"))
    )
    LOGGER.info("Entrando al sistema de creditos")
    opcion_aplicar_credito_boton.click()

    LOGGER.info("Haciendo scroll hacia el final")
    DRIVER.execute_script("window.scrollTo(0, document.body.scrollHeight);")

    selec_option = WebDriverWait(DRIVER, TIMEOUT).until(
    EC.element_to_be_clickable((By.XPATH, "/html/body/div[2]/div[4]/form/div[3]/div/div/div[2]/div[15]/span/a/span[1]"))
    )
    selec_option.click()

    flag_cash = WebDriverWait(DRIVER, TIMEOUT).until(
        EC.element_to_be_clickable((
            By.XPATH,
            "/html/body/ul[16]/li[2]/a"
        ))
    )

    flag_cash.click()
    next = WebDriverWait(DRIVER, TIMEOUT).until(       
    ...
    LOGGER.info("Agregando el tiempo de desplazamiento")
    tiempo_de_desplazamiento = WebDriverWait(DRIVER, TIMEOUT).until(
        EC.element_to_be_clickable((By.XPATH, "/html/body/div[2]/div[4]/form/div[1]/div/div/div[1]/div[2]/input"))
    )

    tiempo_de_desplazamiento.clear()
    tiempo_de_desplazamiento.send_keys("30")
    

    LOGGER.info("Haciendo segundo next")
    next2 = WebDriverWait(DRIVER, TIMEOUT).until(       
        EC.element_to_be_clickable((By.XPATH, '//*[@id="next"]'))
    )
    time.sleep(1)
    next2.click()
    ...
    LOGGER.info("Haciendo tercer next")
    next3 = WebDriverWait(DRIVER, TIMEOUT).until(       
        EC.element_to_be_clickable((By.XPATH, '//*[@id="next"]'))
    )
    next3.click()

    LOGGER.info("Haciendo cuarto next")
    next4 = WebDriverWait(DRIVER, TIMEOUT).until(       
        EC.element_to_be_clickable((By.XPATH, '//*[@id="next"]'))
    )
    next4.click()

    LOGGER.info("Haciendo quinto next")
    next5 = WebDriverWait(DRIVER, TIMEOUT).until(       
        EC.element_to_be_clickable((By.XPATH, '//*[@id="next"]'))
    )
    next5.click()

    LOGGER.info("Haciendo sexto next")
    next6 = WebDriverWait(DRIVER, TIMEOUT).until(       
        EC.element_to_be_clickable((By.XPATH, '//*[@id="next"]'))
    )
    next6.click()
    ...
    LOGGER.info('Dando click en ok')
    ok = WebDriverWait(DRIVER, TIMEOUT).until(       
        EC.element_to_be_clickable((By.XPATH, '/html/body/div[2]/div[4]/div[2]/a/button'))
    )
    ok.click()

    LOGGER.info('Rellenando la información de planes')
    plan_info1 = WebDriverWait(DRIVER, TIMEOUT).until(       
        EC.element_to_be_clickable((By.XPATH, '/html/body/div[2]/div[4]/form/div[2]/div/div/div[1]/h4[7]/span/a/span[1]'))
    )
    plan_info1.click()

    time.sleep(1)
    plan_info2 = WebDriverWait(DRIVER, TIMEOUT).until(       
        EC.element_to_be_clickable((By.XPATH, '/html/body/ul[1]/li[8]/a'))
    )
    plan_info2.click()

    time.sleep(1)
    plan_info3 = WebDriverWait(DRIVER, TIMEOUT).until(       
        EC.element_to_be_clickable((By.XPATH, '/html/body/ul[1]/li[8]/a'))
    )
    plan_info3.click()

    time.sleep(1)
    plan_info4 = WebDriverWait(DRIVER, TIMEOUT).until(       
        EC.element_to_be_clickable((By.XPATH, '/html/body/div[2]/div[4]/form/div[2]/div/div/div[1]/div/h4/span/a/span[1]'))
    )
    plan_info4.click()

    time.sleep(1)
    plan_info5 = WebDriverWait(DRIVER, TIMEOUT).until(       
        EC.element_to_be_clickable((By.XPATH, '/html/body/ul[2]/li[2]/a'))
    )
    plan_info5.click()
    
    time.sleep(1)
    plan_info6 = WebDriverWait(DRIVER, TIMEOUT).until(       
        EC.element_to_be_clickable((By.XPATH, '//*[@id="next"]'))
    )
    plan_info6.click()
    ...
    ok = WebDriverWait(DRIVER, TIMEOUT).until(       
        EC.element_to_be_clickable((By.XPATH, '//*[@id="unc-success-btn"]'))
    )
    ok.click()
    ...
    time.sleep(1)
    plan_info7 = WebDriverWait(DRIVER, TIMEOUT).until(       
        EC.element_to_be_clickable((By.XPATH, '//*[@id="depositAmount"]'))
    )
    plan_info7.click()

    time.sleep(1)
    plan_info8 = WebDriverWait(DRIVER, TIMEOUT).until(       
        EC.element_to_be_clickable((By.XPATH, '//*[@id="depositAmountBtn"]'))
    )
    plan_info8.click()
...
def modulo_8_aplicar_descuento_empleado(empleado):
    LOGGER.info("=== MÓDULO 8: APLICAR DESCUENTO EMPLEADO ==========================================================")

    # -------------------------------------------------------
    # JAVASCRIPT: buscar modal override y presionar YES
    # -------------------------------------------------------
    JS_BUSCAR_MODAL = """
        function clickYesInAllFrames() {
            const selectorModal = "div#transactiondiscountOverrideDialogs";
            const selectorYes = "#yesButton1";

            function searchHere(doc) {
                try {
                    const modal = doc.querySelector(selectorModal);
                    if (modal && modal.style.display !== "none") {
                        const yesBtn = modal.querySelector(selectorYes);
                        if (yesBtn) {
                            yesBtn.click();
                            return true;
                        }
            ...
            function searchFrames(win) {
                ...
                for (let i = 0; i < iframes.length; i++) {
                    try {
                        ...
                        if (childWin) {
                            if (searchFrames(childWin)) return true;
                        }
                    } catch (e) {}
                }
                return false;
            ...
            return searchFrames(window);
        }

        return clickYesInAllFrames();
    """

    # -------------------------------------------------------
    # 1. Menú de transacción
    # -------------------------------------------------------
    Menu_descuento = WebDriverWait(DRIVER, TIMEOUT).until(
        EC.element_to_be_clickable((By.ID, "transactionOptionsBtn"))
    )
    Menu_descuento.click()

    seleccionar_descuento = WebDriverWait(DRIVER, TIMEOUT).until(
        EC.element_to_be_clickable((By.ID, "employeeDiscountLink"))
    )
    seleccionar_descuento.click()

    # -------------------------------------------------------
    # 2. JS override
    # -------------------------------------------------------
    try:
        DRIVER.execute_script(JS_BUSCAR_MODAL)
    except:
        pass
    ...
    # 3. ID empleado
    # -------------------------------------------------------
    id_empleado = WebDriverWait(DRIVER, TIMEOUT).until(
        EC.element_to_be_clickable((By.XPATH, "/html/body/div[2]/div[3]/div[10]/div[2]/form/input[2]"))
    )
    id_empleado.clear()
    id_empleado.send_keys(str(empleado))

    aplicar_empleado = WebDriverWait(DRIVER, TIMEOUT).until(
        EC.element_to_be_clickable((By.XPATH, "/html/body/div[2]/div[3]/div[10]/div[2]/form/button"))
    )
    aplicar_empleado.click()

    # -------------------------------------------------------
    # 4. Esperar aparición real de los inputs duplicados
    # -------------------------------------------------------
    LOGGER.info("Buscando input de porcentaje (IDs duplicados)...")

    WebDriverWait(DRIVER, TIMEOUT).until(
        EC.presence_of_all_elements_located((By.CSS_SELECTOR, "#discountPercentInput"))
    )

    time.sleep(5)

    # -------------------------------------------------------
    # 5. JavaScript ROBUSTO: modificar SOLO el input del modal visible
    # -------------------------------------------------------
    JS_SET_PERCENT = """
        let inserted = false;

        const modals = Array.from(document.querySelectorAll("div.modal, div.discountItemDialog"));

        modals.forEach(m => {
            const visible = window.getComputedStyle(m).display !== "none";
            if (visible) {
                const inputs = m.querySelectorAll("#discountPercentInput, .discountDialogInput, input[name='itemParamString']");
    ...
    porcentaje_ok = DRIVER.execute_script(JS_SET_PERCENT)
    time.sleep(5)
    

    if porcentaje_ok:
        LOGGER.info("Porcentaje aplicado correctamente en el modal visible (10%)")
    else:
        LOGGER.warning("⚠ No se pudo insertar el porcentaje, revisar el DOM")

    time.sleep(1)

    # -------------------------------------------------------
    # 6. Botón confirmar vía JavaScript
    # -------------------------------------------------------
    boton_confirmar = WebDriverWait(DRIVER, TIMEOUT).until(
        EC.presence_of_element_located((By.XPATH, "/html/body/div[2]/div[4]/div[2]/form/p/button"))
    )

    DRIVER.execute_script("arguments[0].click();", boton_confirmar)
    LOGGER.info("Descuento confirmado vía JS")
...
def agregar_descuento_empleado(con_descuento):
    ...
    try:
        ...
        if con_descuento:
            ...
            LOGGER.info("Descuento empleado omitido (configurado como No)")
...
def agregar_creditos(con_creditos):
    ...
    try:
        ...
        if con_creditos:
            ...
            LOGGER.info("Créditos omitido (configurado como No)")
                            ...
                            if is_visible:
                                ...
                                try:
                                    ...
                                    if boton_print:
                                        ...
                                        try:
                                            confirmacion = WebDriverWait(DRIVER, TIMEOUT).until(
                                                EC.element_to_be_clickable((By.XPATH, "/html/body/div[4]/div[2]/div/form/button"))
                                            )
...
def agregar_hipervinculo_word(archivo_excel, filas_cotizacion, nombre_archivo):
    """
    Convierte el valor de ARCHIVO_WORD en un hipervínculo relativo
    ...
    try:
        ...
        for idx, col in enumerate(ws[1], start=1):
            if col.value == "ARCHIVO_WORD":
                col_archivo = idx
                break

        if not col_archivo:
            LOGGER.error("No se encontró columna ARCHIVO_WORD para el hipervínculo")
            return
        
        # Agregar el hipervínculo relativo en cada fila
        for fila in filas_cotizacion:
            excel_row = fila + 2  # +2 porque pandas es 0-index y Excel empieza en 1 + encabezado
            celda = ws.cell(row=excel_row, column=col_archivo)
            celda.value = nombre_archivo
            celda.hyperlink = nombre_archivo  # relativo al archivo Excel
            celda.font = Font(color="0000FF", underline="single")

        wb.save(archivo_excel)
        LOGGER.info("Hipervínculos agregados correctamente")

    except Exception as e:
        LOGGER.error(f"Error agregando hipervínculo: {e}")
...
def limpiar_recursos():
    ...
    try:
        ...
        if SCREENSHOTS_TOMADAS:
            screenshots_eliminados = 0
            LOGGER.info("Eliminando screenshots residuales...")
            for archivo, descripcion in SCREENSHOTS_TOMADAS:
                if os.path.exists(archivo):
                    try:
                        os.remove(archivo)
...
def main():
    """
    MÉTODO PRINCIPAL - Flujo completo paso a paso
    """
    if len(sys.argv) != 2:
    ...
    try:
        ...
        # PASO 1: LECTURA EXCEL
        LOGGER.info("=== PASO 1: LECTURA EXCEL ===")
        cotizaciones = leer_excel_cotizaciones(archivo_excel)
        if not cotizaciones:
            return
        
        # PASO 2: INICIALIZAR NAVEGADOR Y SESIÓN
        LOGGER.info("=== PASO 2: INICIALIZAR NAVEGADOR ===")
        if not iniciar_navegador():
            return
        
        # PASO 3: INICIAR SESIÓN
        LOGGER.info("=== PASO 3: INICIAR SESIÓN ===")
        if not iniciar_sesion():
            LOGGER.error("Error en inicio de sesión - No se puede continuar con automatización")
            LOGGER.info("Como no hay cotizaciones procesadas, saltando generación de reportes")
            return
        
        # Procesar cada cotización individualmente
        for i, (cotizacion_id, productos) in enumerate(cotizaciones.items(), 1):
            
            LOGGER.info(f"\n🔄 PROCESANDO COTIZACIÓN {i}/{len(cotizaciones)}: {cotizacion_id}")
            
            # Establecer cotización actual para screenshots
            global COTIZACION_ACTUAL
            COTIZACION_ACTUAL = cotizacion_id
            
            # PASO 4: INICIAR ORDEN DE COMPRA
            LOGGER.info("=== PASO 4: INICIAR ORDEN DE COMPRA ===")
            if not iniciar_orden_compra():
                LOGGER.warning(f"No se pudo iniciar orden de compra para cotización {cotizacion_id} - continuando con siguiente")
                continue
            ...
            config = productos[0]  # Primer producto tiene la config
            con_garantia = config.get('con_garantia')
            descuento_empleado = config.get('descuento_empleado')  
            con_creditos = config.get('con_creditos')
            entrega_cliente = config.get('entrega_cliente')
            tipo_entrega = config.get('tipo_entrega')
            empleado_id = config.get('empleado_id')
            ...
            # PASO 5: EXTRAER NÚMERO DE COTIZACIÓN
            LOGGER.info("=== PASO 5: EXTRAER NÚMERO COTIZACIÓN ===")
            ...
            # PASO 6: AGREGAR ITEMS Y GARANTÍAS
            LOGGER.info("=== PASO 6: AGREGAR ITEMS ============================================================================")
            for j, producto in enumerate(productos, 1):
                LOGGER.info(f"--- ITEM {j}/{len(productos)} ---")
                
                # Extraer código y cantidad
                ...
                # Agregar item
                if not agregar_item(codigo, cantidad, j):
                    LOGGER.warning(f"Item {j} falló, continuando...")
                    continue
                
                # Agregar garantías para este item
                garantia_item = producto.get('con_garantia', con_garantia)
                if not agregar_garantias(garantia_item, j):
                    LOGGER.warning(f"Garantías item {j} fallaron")
            
            # PASO 7: ENLAZAR CUSTOMER
            LOGGER.info("=== PASO 7: ENLAZAR CUSTOMER ==========================================================================")
            if not enlazar_customer():
                LOGGER.error(" Error enlazando customer")
                continue
            
            
            if(entrega_cliente =='SI'):
                # PASO 8: CONFIGURAR ENTREGA/DESPACHO
                LOGGER.info("=== PASO 8: CONFIGURAR ENTREGA ====================================================================")
                if not agregar_despacho(entrega_cliente, tipo_entrega):
                    LOGGER.warning("⚠️ Error en despacho, continuando...")
                
            # PASO 9: APLICAR DESCUENTO EMPLEADO
            

            if(con_creditos =='SI'):
                #entra solo se se le ha dado la indicacion de en
                LOGGER.info("=== PASO 9: DESCUENTO EMPLEADO ====================================================================")
                modulo_7_aplicar_credito()


              # Función deshabilitada
            
            
            if(descuento_empleado =='SI'):
                # PASO 10: APLICAR CRÉDITOS
                LOGGER.info("=== PASO 10: CRÉDITOS =============================================================================")
                modulo_8_aplicar_descuento_empleado(empleado_id)  # Función deshabilitada
            
            # PASO 11: FINALIZAR COTIZACIÓN
            LOGGER.info("=== PASO 11: FINALIZAR COTIZACIÓN ===")
            ...
            if isinstance(resultado_finalizacion, bool):
                finalizacion_exitosa = resultado_finalizacion
                if resultado_finalizacion:
                    LOGGER.info("✅ Finalización exitosa")
                else:
                    LOGGER.warning("⚠️ Error en finalización, pero continuando con reporte")
            elif isinstance(resultado_finalizacion, str) and not resultado_finalizacion.startswith("ERROR"):
                LOGGER.info(f"Finalización exitosa: {resultado_finalizacion}")
                finalizacion_exitosa = True
            else:
                LOGGER.warning(f"⚠️ Error en finalización: {resultado_finalizacion}, pero continuando con reporte")
            
            # Registrar cotización procesada (siempre, independientemente de finalización)
            if '_' in numero_cotizacion_completo:
                numero_cotizacion_real = numero_cotizacion_completo.split('_')[0]
            ...
            COTIZACIONES_PROCESADAS.append({
                ...
                'numero_cotizacion': numero_cotizacion_real,
                'numero_cotizacion_unico': numero_cotizacion_unico,
                'productos': len(productos),
                'descuento_empleado': descuento_empleado,
                'con_creditos': con_creditos,
                'entrega_cliente': entrega_cliente,
                'finalizado_exitosamente': finalizacion_exitosa
            })
            
            # PASO 12: GENERAR REPORTE INDIVIDUAL (SIEMPRE)
            LOGGER.info("=== PASO 12: GENERAR REPORTE INDIVIDUAL ===")
            try:
                LOGGER.info(f"Generando reporte para cotización {cotizacion_id} con número {numero_cotizacion_real}")
                LOGGER.info(f"Screenshots disponibles: {len(SCREENSHOTS_TOMADAS)}")
                generar_reporte_individual(cotizacion_id, numero_cotizacion_real, numero_cotizacion_unico, productos, entrega_cliente, tipo_entrega)
                LOGGER.info(f"Reporte Word generado exitosamente para cotización {cotizacion_id}")
                
                # Limpiar screenshots después de generar el reporte
                LOGGER.info("Iniciando limpieza de screenshots después de generar reporte")
                limpiar_screenshots_cotizacion()
                
            except Exception as e:
                LOGGER.error(f"Error generando reporte Word: {e}")
                import traceback
                LOGGER.error(f"Traceback: {traceback.format_exc()}")
        ...
        # PASO 13: CREAR REPORTES FINALES
        LOGGER.info("=== PASO 13: CREAR REPORTES FINALES ===")
