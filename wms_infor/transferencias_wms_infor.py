...
def esperar_finalizacion_trabajo(base,numero_orden,conexion,logger,tabla_origen,tabla_destino,job_name,timeout_minutos):
    ...
    logger.info(f"Iniciando monitoreo | Orden={numero_orden} | Origen={tabla_origen} a Destino={tabla_destino}")
...
def validar_inventario(df_inv_inicial, df_inv_final, df_orden, logger):
    ...
    try:
        ...
        df_merged = df_validacion_origen.merge(df_validacion_destino, on='SKU', how='left', suffixes=('_orig', '_dest'))
        
        for _, row in df_merged.iterrows():
            destino_val = str(row['DESTINO']) if pd.notna(row.get('DESTINO')) else ''
            inv_destino_val = int(row['INV_INI_dest']) if pd.notna(row.get('INV_INI_dest')) else 0
            destino_ok = bool(row['VALIDO_dest']) if pd.notna(row.get('VALIDO_dest')) else True
            estado = 'OK' if (row['VALIDO_orig'] and destino_ok) else 'ERROR'
            ...
            movimiento = {
                ...
                'inv_origen': int(row['INV_INI_orig']),
                'inv_destino': inv_destino_val,
                'origen_ok': bool(row['VALIDO_orig']),
...
def enviar_correo_sin_transacciones(logger):
    ...
    enviado = NotificadorCorreo(flujo='wms_infor').enviar(asunto, cuerpo)
...
def enviar_correo_resumen_validacion(lista_resultados, logger=None):
    ...
    return NotificadorCorreo(flujo='wms_infor').enviar(asunto, cuerpo, adjuntos=adjuntos_totales)

def procesar_transferencias_wms_infor(BASE='RI14DB'):
    """
    ...
    try:
        ...
        query_ordenes = f"SELECT DISTINCT trim(NUMORDEN) as NUMORDEN FROM {BASE}.KTCHPWASIN a WHERE STTTD > 0"
        LOGGER.info(f"query: {query_ordenes}")
        ...
        reporte_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'Reportes', 'Transferencias_WMS_INFOR')
        ...
        for idx, row in df_ordenes.iterrows():
            ...
            if word_path:
                ...
                jira = JiraClient()
                jira.main_jira(numero_orden, reporte_path, tipo, "RI14DB")
