        ...
        if os.path.exists(destinatarios_path):
            try:
                with open(destinatarios_path, 'r', encoding='utf-8') as f:
                    ...
                    destinatarios = config_flujos.get('wms_infor', [])
                print(f"Destinatarios encontrados en archivo: {destinatarios}")
            except Exception as e:
                print(f"Error leyendo JSON de destinatarios: {e}")
        
        if not destinatarios:
            print("Archivo de destinatarios no encontrado o vacío, usando destinatario por defecto")
