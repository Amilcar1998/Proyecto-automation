...
try:
    # Opción A: Si ejecutas desde el main.py (fuera de la carpeta validacion_db)
    from validacion_db.transferencias_wms_infor import enviar_correo
except ImportError:
    try:
        # Opción B: Si ejecutas directamente el archivo o usas import relativo
        from transferencias_wms_infor import enviar_correo
    except ImportError:
        # Opción C: Si estás usando imports relativos de Python (.)
        from .transferencias_wms_infor import enviar_correo
...
def generar_reporte_validacion_word(resultado_validacion, output_dir="reportes"):
    ...
    tipo ='PO'
    jira = JiraClient()
    jira.main_jira(orden,ruta_relativa,tipo)
...
def procesar_orden_krws(base, numero_orden, logger=None):
    ...
    try:
        ...
        esperar_finalizacion_trabajoPO(
            base, numero_orden, conexion, log,
            tabla_origen="KRWSPWASIN", tabla_destino="KRWSPWINRC",
            job_name=f"WMSI_PO{cod_pais}", timeout_minutos="15"
        )
...
def main_PO():
    BASE = "RI12DB"          # Base AS400
    LIMITE = None      # None = todas las órdenes
