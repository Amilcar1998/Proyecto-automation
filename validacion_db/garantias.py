...
class GARANTIAS:
    def __init__(self):
        ...
        self.logger.info("Iniciando instancia de GARANTIAS")
        ...
        try:
            ...
            if not self.conexion:
                ...
                if self.conexion:
                    ...
                    self.logger.info("✅ Conexión establecida exitosamente (autocommit=True)")
                    return True
                else:
            ...
            if self.conexion:
                ...
                self.logger.info("✅ Reconexión exitosa (autocommit=False)")
                return True
            else:
                self.logger.error("❌ Fallo en la reconexión")
                return False
    ...
    def setup_logging(self):
        ...
        log_file = os.path.join(logs_dir, f'garantias_{fecha}.log')
        
        logger = get_logger(__file__)

        # Añadir file handler si no existe
    ...
    def copiar_objeto(self, base):
        ...
        def _accion(cursor):
            ...
            cursor.execute("CALL QSYS2.QCMDEXC(?)", (comando,))
            return True
        ...
        print(f"Error ejecutando CPYF hacia {base}/KREGP")
        return False
    ...
    def delete_kregp(self, base_RI):
        self.logger.info(f"Iniciando eliminación de registros en ELOPEZ.KREGP para base destino {base_RI}")
        sql = "DELETE FROM ELOPEZ.KREGP"

        def _action(cursor):
            self.logger.info("Ejecutando DELETE sobre ELOPEZ.KREGP")
            cursor.execute(sql)
        ...
        if registros_eliminados is None:
            self.logger.error(f"Error al eliminar datos de ELOPEZ.KREGP para base destino {base_RI}")
            print(f"Error al eliminar datos de ELOPEZ.KREGP para base destino {base_RI}")
            return False

        self.logger.info(f"Eliminados {registros_eliminados} registros de ELOPEZ.KREGP")
        print("Datos eliminados de ELOPEZ.KREGP correctamente.")
        return True
        ...
        try:
            ...
            if not os.path.exists(cantidad_path):
                ...
                with open(cantidad_path, 'w', encoding='utf-8') as f:
                    ...
                    f.write("200\n")
                print(f"✅ Archivo de configuración creado en: {cantidad_path}")
                return 200
            
            print(f"✅ Archivo de configuración encontrado: {cantidad_path}")
                ...
                for line in f:
                    ...
                    if line and not line.startswith('#'):
                        try:
                            cantidad = int(line)
                            if cantidad > 0:
                                print(f"📋 Cantidad leída desde configuración: {cantidad}")
                                return cantidad
                        except ValueError:
                            continue
            
            # Si no se encuentra valor válido, usar 200 por defecto
            print("⚠️ No se encontró valor válido, usando 200 por defecto")
            return 200
            
        except Exception as e:
            print(f"❌ Error leyendo configuración de cantidad: {e}")
            return 200
    ...
    def copiar_objeto(self,base):
        ...
        print("Error ejecutando CPYF hacia RI11DB/KREGP")
        return False
    ...
    def main_garantias(self, base_RI, logger=None):
        ...
        try:
            ...
            self.logger.info("PASO 6: Insertando items en ELOPEZ.KREGP")
            resultado_insert = self.insertar_items_kregp(base_RI, homologacion)
            if not resultado_insert:
                self.logger.error("Falló la inserción en ELOPEZ.KREGP")
                return False

            self.logger.info(f"PASO 7: Copiando ELOPEZ.KREGP hacia {base_RI}.KREGP")
            resultado_copia = self.copiar_objeto(base_RI)
            if not resultado_copia:
                self.logger.error(f"Falló el CPYF hacia {base_RI}.KREGP")
                return False
